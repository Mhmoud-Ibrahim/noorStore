...
export default function Register() {
  ...
  async function signup(values: RegisterValues) {
    setLoading(true);
    setErrorMsg(''); // تصغير الخطأ عند المحاولة الجديدة
    try {
      const response = await api.post('/auth/signup', values);
      if (response.data.message === 'success') {
        navigate('/login'); // يفضل التوجيه للوجن بعد التسجيل
      }
    } catch (err: any) {
      setErrorMsg(err.response?.data?.message || "حدث خطأ ما");
    } finally {
          ...
          <form onSubmit={formik.handleSubmit}>
            <motion.div variants={itemVariants} className="mb-3">
              ...
              <input 
                ...
                type="text" id="name" 
                className={`form-control bg-light text-dark ${formik.errors.name && formik.touched.name ? 'is-invalid' : ''}`} 
              />
              {formik.errors.name && formik.touched.name && <div className='text-danger small'>{formik.errors.name}</div>}
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="mb-3">
              ...
              <input 
                ...
                type="email" id="email" 
                className={`form-control bg-light text-dark ${formik.errors.email && formik.touched.email ? 'is-invalid' : ''}`} 
              />
              {formik.errors.email && formik.touched.email && <div className='text-danger small'>{formik.errors.email}</div>}
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="mb-4">
              ...
              <div className="position-relative">
                <input 
                  ...
                  type={showPassword ? "text" : "password"} 
                  id="password" 
                  className={`form-control bg-light text-dark ${formik.errors.password && formik.touched.password ? 'is-invalid' : ''}`} 
                />
                <span onClick={togglePassword} style={{ cursor: 'pointer', position: 'absolute', right: '10px', top: '7px' }}>
                  <i className={`fa-regular ${showPassword ? 'fa-eye-slash' : 'fa-eye'}`} style={{ color: '#ff6600' }}></i>
                </span>
              </div>
              {formik.errors.password && formik.touched.password && <div className='text-danger small'>{formik.errors.password}</div>}
            </motion.div>
